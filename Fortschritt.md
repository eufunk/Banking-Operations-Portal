...
Alle Abweichungen vom ursprünglichen Plan sind zusätzlich an zwei Stellen dokumentiert: direkt im betroffenen Kapitel in `ProjektErstellung.docx` ("Update / Umsetzung") und als nummerierter Eintrag in [docs/adr/decisions-backlog.md](docs/adr/decisions-backlog.md) (aktuell 22 Einträge).

## Umgesetzt (Kapitel 1–12 und 16–17 von 24)
...
| 17 | Integration Tests | `WebApplicationFactory<Program>` (echte Api In-Process, keine gemockte Application Layer) gegen LocalDB statt Testcontainers (kein Docker auf dieser Maschine installiert – ADR #22), je frisch angelegte Datenbank pro Testlauf. Deckt POST/GET /api/payments, GET /api/customers, GET /api/transactions ab, über die Dimensionen Status Codes/Validation/Persistence/API Response/Error Handling | **Nur Build verifiziert** (`dotnet build`, 0 Fehler). `dotnet test` durch McAfees Anwendungssteuerungsrichtlinie blockiert (blockiert `Banking.IntegrationTests.dll`, dieselbe Art Umgebungsproblem wie bei Kapitel 12) – **Live-Testlauf steht noch aus.** |
...
- **Kapitel 17: Testlauf steht noch aus** (McAfee-Block, siehe oben) – der Code ist fertig, aber nicht in dieser Sitzung tatsächlich grün gelaufen. Sobald möglich: `sqllocaldb start MSSQLLocalDB && dotnet test tests/Banking.IntegrationTests/Banking.IntegrationTests.csproj`.
- **McAfee blockierte im Zuge dessen erneut auch die (bereits erfolgreich getestete) `Banking.UnitTests.dll`** nach einem späteren Neu-Build – reine Umgebungsblockade, der Testcode selbst ist unverändert. Ein erneuter Lauf beider Testprojekte steht aus, sobald McAfee nicht mehr blockiert.
...
| 22 | Architecture Decision Records | **Teilweise vorhanden.** `docs/adr/decisions-backlog.md` mit 22 nummerierten Einträgen dient bisher als informelles ADR-Log. Individuelle, formale ADR-Dokumente (z. B. `docs/adr/0001-....md` im Standardformat) wurden noch nicht angelegt. |
| 23 | Finaler Code Review | Nicht begonnen. |
| 24 | Interview-Simulation | Nicht begonnen. |

## Empfohlene nächste Schritte

1. **Kapitel 17 live verifizieren**, sobald McAfee die neue Testassembly nicht mehr blockiert (`sqllocaldb start MSSQLLocalDB && dotnet test tests/Banking.IntegrationTests/Banking.IntegrationTests.csproj`).
