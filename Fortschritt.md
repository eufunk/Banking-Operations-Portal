...
| 17 | Integration Tests | `WebApplicationFactory<Program>` (echte Api In-Process, keine gemockte Application Layer) gegen LocalDB statt Testcontainers (kein Docker auf dieser Maschine installiert – ADR #22), je frisch angelegte Datenbank pro Testlauf. Deckt POST/GET /api/payments, GET /api/customers, GET /api/transactions ab, über die Dimensionen Status Codes/Validation/Persistence/API Response/Error Handling | Live verifiziert: `dotnet test` – 10/10 grün (~1 s), inklusive echter Migration gegen frische LocalDB-Datenbank. Beim Selbst-Review zusätzlich zwei reale Bugs in der Testinfrastruktur gefunden und behoben (eager gelesene Config-Werte, fehlende JSON-Optionen im Testclient) sowie ein bekannter LocalDB-Alias-Quirk umgangen (siehe [testing.md](docs/architecture/testing.md)) |
...
Chronologisch mit Kapitel 13 (Azure SQL Elastic Pool) fortfahren, mit derselben Transparenz wie bisher: reale, lokal lauffähige Bausteine bauen, wo eine echte Azure-Ressource fehlt, und jede Abweichung in ADR + Kapitel-Notiz dokumentieren.
